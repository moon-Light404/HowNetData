...
def GetTitValue(name):  # 得到机构代码
    ...
    post_data = {
        'queryJson': r'{"Platform":"","DBCode":"SCDB","KuaKuCode":"CJFQ,CCND,CIPD,CDMD,BDZK,CISD,SNAD,CCJD,GXDB_SECTION,CJFN,CCVD","QNode":{"QGroup":[{"Key":"Subject","Title":"","Logic":1,"Items":[{"Title":"主题","Name":"SU","Value":"' + name + '","Operate":"%=","BlurType":""}],"ChildItems":[]}]}}','groupId':'8'}
    ...
    for i in a:
        if i['text'] == name and re.search('\d+', i['value']):
            ...
            return i['value']
    for i in a:#如果真的没有的话
        if  i['text'] in name  and re.search('\d+', i['value']):
            print("机构代码:", i['value'])
            return i['value']
...
school_list = ['清华大学', '北京大学', '中国人民大学', '北京工业大学', '北京理工大学', '北京航空航天大学', '北京化工大学', '北京邮电大学', '对外经济贸易大学', '中国传媒大学',
               ...
               '北京林业大学', '中国农业大学', '北京中医药大学', '华北电力大学', '北京师范大学', '中国地质大学(北京)', '复旦大学', '华东师范大学', '上海外国语大学', '上海大学',
               ...
               '延边大学', '云南大学', '东北农业大学', '东北林业大学', '哈尔滨工业大学', '哈尔滨工程大学', '南京大学', '东南大学', '苏州大学', '河海大学', '中国药科大学',
...
#创建多线程使用threading.Thread(target=)方法来对函数进行多线程创建
...
for i in school_list:
    th_1 = threading.Thread(target=thread_t, args=(i,))
    th_1.start()
